IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[proc_EHS_eHSA0027-13]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[proc_EHS_eHSA0027-13]
GO
...
-- Description:		eHSA0027 - FHB statistics for 2011
-- =============================================
...
CREATE PROCEDURE [dbo].[proc_EHS_eHSA0027-13]
	@Year	int
AS BEGIN
...
-- ---------------------------------------------
	...
	(0, 'eHSA0027-13: Statistics of EHCPs used Smart ICs for making claims as at 31 Dec '+@strYear+', broken down by professions')
...
-- ---------------------------------------------
	...
	--SELECT @ROP = COUNT(1) FROM @SP WHERE Service_Type = 'ROP' 
	...
	SELECT @Total = @ENU+@RCM+@RCP+@RDT+@RMP+@RMT+@RNU+@ROT+@RPT+@RRD--COUNT(1) FROM @SP
...
GRANT EXECUTE ON [dbo].[proc_EHS_eHSA0027-13] TO HCVU
GO
