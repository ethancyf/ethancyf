IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[proc_EHS_eHSD0029_Report_get]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[proc_EHS_eHSD0029_Report_get]
GO
...
-- Description:		Daily Deactivated eHRSS Token Report - eHSD0029
-- =============================================    

CREATE Procedure [proc_EHS_eHSD0029_Report_get]    
...
SET NOCOUNT ON;    
	...
	------------------------------------------------------------------------------------------
	-- Report Helper Field
	DECLARE @Report_ID				VARCHAR(30)	= 'eHSD0029'
	...
	IF @IN_target_period_from IS NULL
		SET @IN_target_period_from = DATEFROMPARTS(DATEPART(YEAR, DATEADD(DAY, -1, GETDATE())) ,DATEPART(MONTH, DATEADD(DAY, -1, GETDATE())), DATEPART(DAY, DATEADD(DAY, -1, GETDATE())))
	ELSE
	...
	IF @IN_target_period_to IS NULL
		SET @IN_target_period_to = DATEFROMPARTS(DATEPART(YEAR, DATEADD(DAY, 0, GETDATE())) ,DATEPART(MONTH, DATEADD(DAY, 0, GETDATE())), DATEPART(DAY, DATEADD(DAY, 0, GETDATE())))
	ELSE
	...
	-------------------------
	-- Header 1
	INSERT INTO #ResultTable1 (Result_Value1, Result_Value2, Result_Value3, Result_Value4, Result_Value5) VALUES ('Reporting Date: ' + CONVERT(VARCHAR(10), @IN_target_period_from, 111),'','','','')														
	...
	BEGIN
		...
		INSERT INTO #ResultTable1 (
			...
			Result_Value5)
		VALUES(
			'There is no record in the reporting date', 
			'', 
			'', 
			'', 
			''
	...
	-------------------------
	-- Header 1
	INSERT INTO #ResultTable2 (Result_Value1, Result_Value2, Result_Value3, Result_Value4, Result_Value5) VALUES ('Reporting Date: ' + CONVERT(VARCHAR(10), @IN_target_period_from, 111),'','','','')														
...
-- =============================================	
	...
	-----------------------
	SELECT	
		CASE WHEN ISNULL(@Result1Count, 0) + ISNULL(@Result2Count, 0) > 0 THEN 'Y' ELSE 'N' END AS 'HaveResult'
	
	IF @Result1Count + @Result2Count > 0
	BEGIN				
		--------------------------
...
GRANT EXECUTE ON [dbo].[proc_EHS_eHSD0029_Report_get] TO HCVU
