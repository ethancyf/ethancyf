...
// This openNewHTML2 Function Only for HCSP
 function openNewHTML2(link)
{
    var blnOpen = true;
    ...
    if (blnOpen)
    {
        var wi = screen.width;
        var he = screen.height;
        var myWin = window.open(link, '', 'width='+wi+',height='+he*0.88+',location=no,directories=no,menubar=no,toolbar=no,scrollbars=yes,status=yes,resizable=yes,left=0,top=0');
        myWin.name = 'eHS_HCSP';
        opener = myWin;
    } 
}
...
// This openNewHTML2 Function Only for HCSP
 function openNewHTML3(link, popname)
{
    var wi = screen.width;
    var he = screen.height;
    var myWin = window.open(link, popname, 'width='+wi+',height='+he*0.88+',location=no,directories=no,menubar=no,toolbar=no,scrollbars=yes,status=yes,resizable=yes,left=0,top=0');
    myWin.focus();
}
    ...
    if (xmlDoc!=null) 
    {
                ...
                //{
                    ...
                    {                    
                        ...
                        if (lang == "ZH")
                        {
                            ...
                            var array_r = new Array(2);
                            array_r[2] = y[i].getElementsByTagName("CN_Date")[0].childNodes[0].nodeValue;
                            array_r[1] = y[i].getElementsByTagName("CN_Time")[0].childNodes[0].nodeValue;
                            array_r[0] = y[i].getElementsByTagName("s_id")[0].childNodes[0].nodeValue;
                        }
                        else
                        {
        ...
        else
        {
                ...
                if (blnU)
                {
                    if (current_dtm < schedule_dtm)
                    {
                        ...
                        if (lang == "ZH")
                        {
                            ...
                            var array_r = new Array(2);
                            array_u[2] = y[i].getElementsByTagName("CN_Date")[0].childNodes[0].nodeValue;
                            array_u[1] = y[i].getElementsByTagName("CN_Time")[0].childNodes[0].nodeValue;
                            array_u[0] = y[i].getElementsByTagName("s_id")[0].childNodes[0].nodeValue;
                        }
                        else
                        {
            ...
            if (array_final.length > 0)
            {
                if (lang == "ZH")
                {
                    document.write("<div class='tableHeading'>緊急系統維護</div>");           
                ...
                if (lang == "CN") {
                    document.write("<div class='tableHeading'>紧急系统维护</div>");
                    document.write("<table style='background-color: #87ceeb' border='0' cellpadding='1' cellspacing='1'width='60%'>");
                    document.write("<tr><td style='font-weight: bold; color: #ffffff;width:250px' align='center'>日期</td><td style='font-weight: bold; color: #ffffff' align='center'>时间</td></tr>");
                }
                else
                {
                    document.write("<div class='tableHeading'>Ad hoc System Maintenance Schedule</div>");           
                    document.write("<table style='background-color: #87ceeb' border='0' cellpadding='1' cellspacing='1'width='60%'>");
