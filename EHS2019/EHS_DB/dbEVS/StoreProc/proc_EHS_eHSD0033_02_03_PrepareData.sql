IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[proc_EHS_eHSD0033_02_03_PrepareData]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[proc_EHS_eHSD0033_02_03_PrepareData]
GO
...
-- Description:		Remove QIV and LAIV (Govt) data and sub-report
-- =============================================
 -- =============================================
-- Modification History
-- CR No.:			I-CRE20-005
...
-- CR No.			INT19-0022
-- Description:		Change tinyint -> int
-- =============================================
-- =============================================    
-- Author:			Winnie SUEN
-- Create date:		04 Oct 2019
-- CR No.			CRE19-001-05 (PPP 2019-20 - Report)
-- Description:		PPPKG daily report - Sub report 02 & 03
-- =============================================  

CREATE Procedure [proc_EHS_eHSD0033_02_03_PrepareData]    
...
--SET @Cutoff_Dtm = '2018-11-20'
	...
	-- Get all SIV of PPPKG with current season
...
EXEC [proc_SymmetricKey_open]  
	...
	CREATE TABLE #Transaction		-- Transaction of PPPKG
	(    			
		voucher_acc_id			char(15),    
		temp_voucher_acc_id		char(15), 		
...
-- ---------------------------------------------    
	...
	VALUES (1, REPLACE('eHS(S)D0033-02: Report on yearly PPP-KG claim transaction by age group and target group ([DATE])', '[DATE]', @current_scheme_desc))    
	...
	OPEN Category_Cursor  
		...
		BEGIN  		
			...
			SET @result4 = (SELECT COUNT(DISTINCT SP_ID) 
							FROM #Transaction 
							WHERE 
								Subsidize_Code IN (@QIV_Subsidize_Code, @LAIV_Subsidize_Code))
			...
			-- QIV --		
			SET @result1 = (SELECT COUNT(1) FROM #Transaction WHERE Subsidize_Code IN (@QIV_Subsidize_Code)) -- AND IsSIV = 1 AND IsCurrentSeason = 1

			-- LAIV --
			SET @result2 = (SELECT COUNT(1) FROM #Transaction WHERE Subsidize_Code IN (@LAIV_Subsidize_Code)) -- AND IsSIV = 1 AND IsCurrentSeason = 1												
...
*/	
	...
	-- Total of QIV (91)
	SET @sql = 'INSERT INTO #ResultTable_02_age (Display_Seq, Display_Code,' + @pivot_table_column_list + ') ' +
				'SELECT 91, ''Total of QIV + LAIV'', ' + @pivot_table_column_sum + ' FROM #ResultTable_02_age WHERE Display_Seq>= 60 AND Display_Seq < 90'
	...
	-- =============================================    
	-- eHS(S)D0033-03: Report on yearly PPPKG claim transaction by school code (Current Season) (QIV)
	-- ============================================= 
	...
	INSERT INTO #ResultTable_03 (_display_seq, _result_value1, _result_value2, _result_value3, _result_value4, _result_value5,
								_result_value6,  _result_value7,  _result_value8,  _result_value9,  _result_value10)
	VALUES (5, '', '', '', '', '', '', '', '', '', '')


	UPDATE #ResultTable_03
	SET _result_value2 = (	SELECT Display_Code_For_claim 
							FROM @SIV_ByCategory 
							WHERE Subsidize_Code LIKE '%QIV')						
	...
	FROM
		(
			...
			FROM
				#Transaction
			WHERE 
				Subsidize_Code = @QIV_Subsidize_Code
			GROUP BY
				SchoolCode
		) T
	--LEFT JOIN
	...
	--		GROUP BY
	--			SchoolCode
	--	) QIV ON T.SchoolCode = QIV.SchoolCode


	-- =============================================    
	-- eHS(S)D0033-03: Report on yearly PPPKG claim transaction by school code (Current Season) (LAIV)
	-- ============================================= 

	-- insert record for the final output format    
	INSERT INTO #ResultTable_04 (_display_seq, _result_value1)    
	VALUES (1, REPLACE('eHS(S)D0033-03: Report on yearly PPP-KG claim transaction by school code ([DATE])', '[DATE]', @current_scheme_desc))    
	INSERT INTO #ResultTable_04 (_display_seq)    
	VALUES (2)    
	INSERT INTO #ResultTable_04 (_display_seq, _result_value1)    
	VALUES (3, 'Reporting period: as at ' + FORMAT(DATEADD(dd, -1, @Cutoff_Dtm), 'yyyy/MM/dd'))
	INSERT INTO #ResultTable_04 (_display_seq)    
	...
	INSERT INTO #ResultTable_04 (_display_seq, _result_value1, _result_value2, _result_value3, _result_value4, _result_value5,
								_result_value6,  _result_value7,  _result_value8,  _result_value9,  _result_value10)
	VALUES (5, '', '', '', '', '', '', '', '', '', '')

	UPDATE #ResultTable_04
	SET _result_value2 = (	SELECT Display_Code_For_claim 
							FROM @SIV_ByCategory 
							WHERE Subsidize_Code LIKE '%LAIV')								
	WHERE _display_seq = 5
	...
	FROM
		(
			...
			FROM
				#Transaction
			WHERE 
				Subsidize_Code = @LAIV_Subsidize_Code
			GROUP BY
				SchoolCode
		) T
	--LEFT JOIN
	...
	--		GROUP BY
	--			SchoolCode
	--	) LAIV ON T.SchoolCode = LAIV.SchoolCode
	...
	DELETE FROM RpteHSD0033_02_PPPKG_Tx_ByAgeGroup_ByYear   
	 
	INSERT INTO RpteHSD0033_02_PPPKG_Tx_ByAgeGroup_ByYear    
	(
		Display_Seq,    
...
--
	...
	DELETE FROM RpteHSD0033_03_PPPKG_QIV_Tx_BySchoolCode
	
	INSERT INTO RpteHSD0033_03_PPPKG_QIV_Tx_BySchoolCode    
	(
		Display_Seq,    
		Col1,  Col2,  Col3,  Col4,  Col5,  
		Col6,  Col7,  Col8,  Col9,  Col10	
...
--
	...
	DELETE FROM RpteHSD0033_03_PPPKG_LAIV_Tx_BySchoolCode
	
	INSERT INTO RpteHSD0033_03_PPPKG_LAIV_Tx_BySchoolCode    
	(
		Display_Seq,    
		Col1,  Col2,  Col3,  Col4,  Col5,  
		Col6,  Col7,  Col8,  Col9,  Col10	
...
--
	...
	DROP TABLE #ResultTable_04
    
END    
GO

GRANT EXECUTE ON [dbo].[proc_EHS_eHSD0033_02_03_PrepareData] TO HCVU
