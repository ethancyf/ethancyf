IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[proc_VoidableVoucherAccount_get_byPartialID_IVRS]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[proc_VoidableVoucherAccount_get_byPartialID_IVRS]
GO
...
CREATE PROCEDURE [dbo].[proc_VoidableVoucherAccount_get_byPartialID_IVRS]   
 @doc_code char(20),  
...
-- VoucherAccount  
 ...
 CROSS APPLY (
   ...
   WHERE VT.[Voucher_Acc_ID] = VA.[Voucher_Acc_ID] AND 
		 VT.[SP_ID] = @SP_ID AND  
 ...
 LEFT OUTER JOIN [VoucherAccountCreationLog] L  
  ON VA.Voucher_Acc_ID = L.Voucher_Acc_ID  
...
GRANT EXECUTE ON [dbo].[proc_VoidableVoucherAccount_get_byPartialID_IVRS] TO HCSP
GO
