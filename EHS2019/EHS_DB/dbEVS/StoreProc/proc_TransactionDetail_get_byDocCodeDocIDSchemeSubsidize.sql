IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[proc_TransactionDetail_get_byDocCodeDocIDSchemeSubsidize]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[proc_TransactionDetail_get_byDocCodeDocIDSchemeSubsidize]
GO
...
-- Modification History  
-- Modified by:     Pak Ho LEE  
-- Modified date: 17 Feb 2010  
-- Description:     Retrieve DOB  
-- =============================================  
-- =============================================  
-- Modification History  
-- Modified by:     Pak Ho LEE  
...
--     DocCode, Identity, Scheme Code, Scheme Seq, SubsidizeCode  
-- =============================================  
-- =============================================  
-- Modification History  
-- Modified by:  Pak Ho LEE  
...
CREATE PROCEDURE [dbo].[proc_TransactionDetail_get_byDocCodeDocIDSchemeSubsidize]   
 @Doc_Code   char(20),  
 @identity   varchar(20),  
 @Scheme_Code  char(10),  
 @Scheme_seq   smallint,  
...
DECLARE @Subsidize_Item_Code char(10)
...
-- Find the Subsidize_Item_Code

SELECT
	@Subsidize_Item_Code = Subsidize_Item_Code
FROM
	Subsidize
WHERE
 ...
 */  
 ...
 SELECT  
  ...
  tmp.[Exact_DOB]  
 FROM  
  @tmpVoucherTransactionDistinct tmp INNER JOIN [TransactionDetail] TD  
   ON tmp.[Transaction_ID] = TD.[Transaction_ID]  
  INNER JOIN [VoucherTransaction] VT   
   ON TD.[Transaction_ID] = VT.[Transaction_ID]  
     
 WHERE  
  (TD.[Scheme_Code] = @Scheme_Code AND TD.[Scheme_Seq] = @Scheme_seq AND TD.[Subsidize_Code] = @Subsidize_Code)  
  OR  
  EXISTS (  
   SELECT [Scheme_Code] FROM [ViewEqvSubsidizeMap] ESM WHERE   
    ESM.[Scheme_Code] = @Scheme_Code AND   
    ESM.[Scheme_Seq] = @Scheme_seq AND  
    ESM.[Subsidize_Item_Code] = @Subsidize_Item_Code AND  
...
GRANT EXECUTE ON [dbo].[proc_TransactionDetail_get_byDocCodeDocIDSchemeSubsidize] TO HCSP
GO

GRANT EXECUTE ON [dbo].[proc_TransactionDetail_get_byDocCodeDocIDSchemeSubsidize] TO HCVU
GO

GRANT EXECUTE ON [dbo].[proc_TransactionDetail_get_byDocCodeDocIDSchemeSubsidize] TO WSEXT
Go