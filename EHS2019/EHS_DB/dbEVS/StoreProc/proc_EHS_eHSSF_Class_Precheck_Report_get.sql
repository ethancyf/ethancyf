...
-- Description:		(1) Display Covid-19 Vaccine Record
--					(2) Delete MMR columns in ¡¥HCW¡¦ worksheet
-- =============================================
-- =============================================
-- Modification History
-- CR# :			I-CRE20-005
...
-- =============================================
	...
	BEGIN
		...
		WHERE Student_File_ID = @Input_Student_File_ID AND v.Subsidize_Item_Code NOT IN ('PV','PV13','MMR','C19')
		...
		SELECT 
			...
			CASE WHEN (DATEDIFF(D, LastC19V.Vaccine_Date, GETDATE()) < 14 
					AND DATEDIFF(D, LastC19V.Vaccine_Date, GETDATE()) > -14) THEN 'Y'
				ELSE 'N'
		...
		-- Join all COVID-19 vaccination record
		LEFT JOIN #COVID19VaccineTT C19V
			ON C19V.Student_Seq = e.Student_Seq
		-- Join latest C19 Vaccine to check within period
		LEFT JOIN #COVID19VaccineRowTT LastC19V	
