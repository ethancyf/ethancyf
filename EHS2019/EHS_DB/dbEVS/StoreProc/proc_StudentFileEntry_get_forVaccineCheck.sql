IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[proc_StudentFileEntry_get_forVaccineCheck]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[proc_StudentFileEntry_get_forVaccineCheck]
GO
...
-- Description:		Get StudentFileEntry for vaccination checking
-- =============================================    

CREATE PROCEDURE [dbo].[proc_StudentFileEntry_get_forVaccineCheck]
...
-- =============================================
	...
	SELECT 
		...
		PI.EC_Serial_No
	FROM
		StudentFileEntry E WITH (NOLOCK)
		INNER JOIN StudentFileHeader H WITH (NOLOCK)
	...
	SELECT 
		...
		H.Practice_Display_Seq,
		H.Service_Receive_Dtm AS [Service_Receive_Dtm_Header],
		H.Service_Receive_Dtm-2 AS [Service_Receive_Dtm_2_Header],
		H.Scheme_Code,
		H.Scheme_Seq,
		H.Dose,
		H.Claim_Upload_By,
...
GRANT EXECUTE ON [dbo].[proc_StudentFileEntry_get_forVaccineCheck] TO HCVU
GO
