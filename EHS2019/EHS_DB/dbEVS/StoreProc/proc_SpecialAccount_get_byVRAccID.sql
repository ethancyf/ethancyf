IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[proc_SpecialAccount_get_byVRAccID]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[proc_SpecialAccount_get_byVRAccID]
GO
...
-- Description:	Retrieve the Special Account
-- =============================================

CREATE PROCEDURE [dbo].[proc_SpecialAccount_get_byVRAccID]
	@VRAccID char(15)
AS
BEGIN
	SET NOCOUNT ON;
...
-- =============================================
	...
	SELECT Transaction_ID, Special_Acc_ID, Scheme_Code
	FROM VoucherTransaction
	WHERE [Special_Acc_ID] = @VRAccID -- and Scheme_Code = @Scheme_Code
	...
	FROM
		...
		left outer join @tmpVoucherTransaction VT on 
			VT.[Special_Acc_ID] = @VRAccID and 
			VA.[Special_Acc_ID] = VT.[Special_Acc_ID] 
			--and VA.[Scheme_Code] = VT.[Scheme_Code]
	WHERE
		VA.[Special_Acc_ID] = @VRAccID
...
GRANT EXECUTE ON [dbo].[proc_SpecialAccount_get_byVRAccID] TO HCSP
GO

GRANT EXECUTE ON [dbo].[proc_SpecialAccount_get_byVRAccID] TO HCVU
GO
