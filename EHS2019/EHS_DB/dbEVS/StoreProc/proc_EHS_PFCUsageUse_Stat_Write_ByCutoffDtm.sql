IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[proc_EHS_PFCUsageUse_Stat_Write_ByCutoffDtm]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[proc_EHS_PFCUsageUse_Stat_Write_ByCutoffDtm]
GO
...
CREATE PROCEDURE [dbo].[proc_EHS_PFCUsageUse_Stat_Write_ByCutoffDtm] 
	@Cutoff_Dtm	datetime
AS BEGIN
-- =============================================
-- Declaration
...
-- ---------------------------------------------
	...
	SELECT
		...
		COUNT(P.Doc_Code)
	FROM
		PreFillPersonalInformation P
			INNER JOIN _tmpPrefillUsage U
				ON P.Pre_Fill_Consent_ID = U.pre_fill_consent_id
	WHERE
		U.used_date 
...
GRANT EXECUTE ON [dbo].[proc_EHS_PFCUsageUse_Stat_Write_ByCutoffDtm] TO HCVU
GO
