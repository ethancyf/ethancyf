...
(
    SELECT *
    FROM dbo.sysobjects
    WHERE id = OBJECT_ID(N'[dbo].[proc_VoucherTransaction_get_byTranAspect]')
          AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
    BEGIN
        DROP PROCEDURE [dbo].[proc_VoucherTransaction_get_byTranAspect];
    END;
GO
...
-- CR No.			CRE20-003
-- Description:		Add School Code (RCH_code->SchoolOrRCH_code), Dose_Code, Subsidize_Item_Code
-- =============================================
-- =============================================
-- Modification History
        ...
        IF @transaction_id IS NULL
            BEGIN
                ...
                (
                    ...
                    WHERE User_ID = @User_ID
                     ...
                     AND (@Subsidize_Item_Code IS NULL
                          OR @Subsidize_Item_Code = TD.Subsidize_Item_Code)
                     AND (@Dose_Code IS NULL
                          OR @Dose_Code = TD.Available_Item_Code)
            ...
            BEGIN
                ...
                FROM VoucherTransaction AS VT WITH(NOLOCK)
                     ...
                     ON VT.Transaction_ID = TAFSC.Transaction_ID
                        AND TAFSC.AdditionalFieldID = 'SchoolCode'
                WHERE(@transaction_id = VT.Transaction_ID)
...
GRANT EXECUTE ON [dbo].[proc_VoucherTransaction_get_byTranAspect] TO HCVU;
GO