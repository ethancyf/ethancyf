...
Namespace WebService.Interface
    ...
    <Serializable()> Public Class PCDTransferPracticeInfoResult
        Inherits BaseResult
        ...
        Public Enum enumReturnCode
            ...
            ServiceProviderNotExistNorActive = 1

            ' Service provider under amendment
            ServiceProviderUnderAmendment = 2

            ' Service Provider Not Yet Activated
            ServiceProviderNotYetActivated = 3
        ...
        Public Sub New(ByVal strPCDTransferPracticeInfoResultXml As String)
            ReadXml(strPCDTransferPracticeInfoResultXml, Nothing)
        End Sub

        Public Sub New(ByVal strPCDTransferPracticeInfoResultXml As String, ByVal strMessageID As String)
            ReadXml(strPCDTransferPracticeInfoResultXml, strMessageID)
        End Sub
        ...
        ''' </summary>
        ''' <param name="strPCDTransferPracticeInfoResultXml">Xml result from PCD web service</param>
        ''' <param name="strMessageID">Xml request message id</param>
        ''' <remarks></remarks>
        Private Sub ReadXml(ByVal strPCDTransferPracticeInfoResultXml As String, ByVal strMessageID As String)
            Dim ds As DataSet = Nothing
            Dim dt As DataTable = Nothing

            _strResult = strPCDTransferPracticeInfoResultXml

            Try
                ds = Common.ComFunction.XmlFunction.Xml2Dataset(strPCDTransferPracticeInfoResultXml)
            ...
            '    ThrowException("PCDTransferPracticeInfoResult: Fail to convert <message_datetime> value to datetime")
            'End If
        ...
        Public Overloads Function getReturnCodeDesc(ByVal enumN As enumReturnCode) As String
            MyBase.GenReturnCodeSystemMessage(EnumConstant.EnumMappingCodeType.WS_PCD_TransferPracticeInfo_Return_Code, enumN)
            ...
            '        udtCodeMap = udtCodeMapList.GetMappingByCode(CodeMappingModel.EnumSourceSystem.PCD, CodeMappingModel.EnumTargetSystem.EHS, CodeMappingModel.EnumCodeType.WS_PCD_TransferPracticeInfo_Return_Code, "0")
            '        _sysMsg = New SystemMessage(strFunctCode, SEVERITY_CODE, udtCodeMap.CodeTarget)
            '        strreturnCodeDesc = _sysMsg.GetMessage()
            '    Case enumReturnCode.ServiceProviderNotExistNorActive
            '        udtCodeMap = udtCodeMapList.GetMappingByCode(CodeMappingModel.EnumSourceSystem.PCD, CodeMappingModel.EnumTargetSystem.EHS, CodeMappingModel.EnumCodeType.WS_PCD_TransferPracticeInfo_Return_Code, "1")
            '        _sysMsg = New SystemMessage(strFunctCode, SEVERITY_CODE, udtCodeMap.CodeTarget)
            '        strreturnCodeDesc = _sysMsg.GetMessage()
            '    Case enumReturnCode.ServiceProviderUnderAmendment
            '        udtCodeMap = udtCodeMapList.GetMappingByCode(CodeMappingModel.EnumSourceSystem.PCD, CodeMappingModel.EnumTargetSystem.EHS, CodeMappingModel.EnumCodeType.WS_PCD_TransferPracticeInfo_Return_Code, "2")
            '        _sysMsg = New SystemMessage(strFunctCode, SEVERITY_CODE, udtCodeMap.CodeTarget)
            '        strreturnCodeDesc = _sysMsg.GetMessage()
            '    Case enumReturnCode.InvalidParameter
            '        udtCodeMap = udtCodeMapList.GetMappingByCode(CodeMappingModel.EnumSourceSystem.PCD, CodeMappingModel.EnumTargetSystem.EHS, CodeMappingModel.EnumCodeType.WS_PCD_TransferPracticeInfo_Return_Code, "98")
            '        _sysMsg = New SystemMessage(strFunctCode, SEVERITY_CODE, udtCodeMap.CodeTarget)
            '        strreturnCodeDesc = _sysMsg.GetMessage()
            '    Case enumReturnCode.ErrorAllUnexpected
            '        udtCodeMap = udtCodeMapList.GetMappingByCode(CodeMappingModel.EnumSourceSystem.PCD, CodeMappingModel.EnumTargetSystem.EHS, CodeMappingModel.EnumCodeType.WS_PCD_TransferPracticeInfo_Return_Code, "99")
            '        _sysMsg = New SystemMessage(strFunctCode, SEVERITY_CODE, udtCodeMap.CodeTarget)
            '        strreturnCodeDesc = _sysMsg.GetMessage()
            '    Case enumReturnCode.CommunicationLinkError
            '        udtCodeMap = udtCodeMapList.GetMappingByCode(CodeMappingModel.EnumSourceSystem.PCD, CodeMappingModel.EnumTargetSystem.EHS, CodeMappingModel.EnumCodeType.WS_PCD_TransferPracticeInfo_Return_Code, "101")
            '        _sysMsg = New SystemMessage(strFunctCode, SEVERITY_CODE, udtCodeMap.CodeTarget)
            '        strreturnCodeDesc = _sysMsg.GetMessage()
            '    Case Else
            '        udtCodeMap = udtCodeMapList.GetMappingByCode(CodeMappingModel.EnumSourceSystem.PCD, CodeMappingModel.EnumTargetSystem.EHS, CodeMappingModel.EnumCodeType.WS_PCD_TransferPracticeInfo_Return_Code, "99")
