IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[proc_StudentFileHeader_search_Rectification]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[proc_StudentFileHeader_search_Rectification]
GO
...
CREATE PROCEDURE [dbo].[proc_StudentFileHeader_search_Rectification]
...
-- =============================================
	...
	WHERE
		(@Student_File_ID IS NULL OR S.Student_File_ID = @Student_File_ID)
			...
			AND S.Record_Status IN ('FR', 'CR', 'UT')
...
GRANT EXECUTE ON [dbo].[proc_StudentFileHeader_search_Rectification] TO HCVU
GO
*/
