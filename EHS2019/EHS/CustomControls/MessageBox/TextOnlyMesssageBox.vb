...
<DefaultProperty("Text"), ToolboxData("<{0}:TextOnlyMessageBox runat=server></{0}:TextOnlyMessageBox>")> _
Public Class TextOnlyMessageBox
    Inherits WebControl
    ...
    Private Const VS_TEXTONLYMESSAGEBOX_CODETABLE As String = "VS_TEXTONLYMESSAGEBOX_CODETABLE"

    Private Const VS_TEXTONLYMESSAGEBOX_HEADERKEY As String = "VS_TEXTONLYMESSAGEBOX_HEADERKEY"

    Private Const VS_TEXTONLYMESSAGEBOX_SHOWED As String = "VS_TEXTONLYMESSAGEBOX_SHOWED"

    Private Const VS_TEXTONLYMESSAGEDESCBOX As String = "VS_TEXTONLYMESSAGEDESCBOX"
    ...
    Private Sub TextOnlyMessageBox_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        'td.Style.Item("width") = "400px"
        lblMsgMainStatement = New Label
        lblMsgMainStatement.ID = "lblMsgMainStatement"
        ...
        td = New TableCell
        'td.Style.Item("width") = "350px"
        td.VerticalAlign = VerticalAlign.Top
        td.HorizontalAlign = WebControls.HorizontalAlign.Left
        dlMsgDetails = New DataList
        dlMsgDetails.ID = "dlMsgDetails"
        ...
        dlMsgDetails.Width = Unit.Percentage(100)
        ...
        pnl.Style.Add("BORDER-RIGHT", "red 1px solid")
        pnl.Style.Add("BORDER-TOP", "red 1px solid")
        pnl.Style.Add("BORDER-LEFT", "red 1px solid")
        pnl.Style.Add("BORDER-BOTTOM", "red 1px solid")
        'pnl.Width = Unit.Percentage(75)
        ...
        'Dim li1 As New LiteralControl("<br>")
        'Controls.Add(li1)
    ...
    ''' <remarks></remarks>
    Public Sub Clear()
        ViewState(VS_TEXTONLYMESSAGEBOX_CODETABLE) = Nothing
        ViewState(VS_TEXTONLYMESSAGEBOX_HEADERKEY) = Nothing
        ViewState(VS_TEXTONLYMESSAGEBOX_SHOWED) = Nothing
        Me.Visible = False
    End Sub
    ...
    Public Function GetCodeTable() As DataTable
        Dim dtCode As DataTable
        If Not ViewState(VS_TEXTONLYMESSAGEBOX_SHOWED) Is Nothing Then
            If Not CBool(ViewState(VS_TEXTONLYMESSAGEBOX_SHOWED)) Then
                dtCode = CType(ViewState(VS_TEXTONLYMESSAGEBOX_CODETABLE), DataTable)
                Return dtCode
            End If
        End If
    ...
    '==================================================================== Code for SmartID ============================================================================
    Private Sub AddCodeTableRowWithDesc(ByRef dtCode As DataTable, ByVal strFunctionCode As String, ByVal strErrorCode As String, ByVal strMsg As String)
        Dim drCode As DataRow
        drCode = dtCode.NewRow
    ...
    Public Sub AddMessageDesc(ByVal strFunctionCode As String, ByVal strErrorCode As String, ByVal strErrorMsg As String)
        ...
        Else
            dtCode = CType(ViewState(VS_TEXTONLYMESSAGEBOX_CODETABLE), DataTable)
        End If
        AddCodeTableRowWithDesc(dtCode, strFunctionCode, strErrorCode, strErrorMsg)
        ViewState(VS_TEXTONLYMESSAGEBOX_SHOWED) = False
        ViewState(VS_TEXTONLYMESSAGEBOX_CODETABLE) = dtCode
    End Sub
    '==================================================================================================================================================================
    ...
    Private Sub DisplayMessageBox(Optional ByVal strHeaderKey As String = "")
        ...
        headertext = ""
        Me.Visible = False
        If Not ViewState(VS_TEXTONLYMESSAGEBOX_CODETABLE) Is Nothing Then

            ViewState(VS_TEXTONLYMESSAGEDESCBOX) = Nothing
            ViewState(VS_TEXTONLYMESSAGEBOX_SHOWED) = True
            ...
            If dtCode.Rows.Count > 0 Then
                ...
                If strHeaderKey <> "" Then
                    ...
                    ViewState(VS_TEXTONLYMESSAGEBOX_HEADERKEY) = strHeaderKey
                    ...
                    End If
                Else
                    If Not ViewState(VS_TEXTONLYMESSAGEBOX_HEADERKEY) Is Nothing Then
                        strHeaderKey = CStr(ViewState(VS_TEXTONLYMESSAGEBOX_HEADERKEY))
                        headertext = CStr(HttpContext.GetGlobalResourceObject("Text", strHeaderKey))
                    End If
                End If
    ...
    Public Sub BuildMessageBox(Optional ByVal strHeaderKey As String = "")
        If Me.GetCodeTable.Rows.Count > 0 Then
            ...
            System.Web.UI.ScriptManager.RegisterStartupScript(Me, Me.GetType, "TextOnlyMessageBoxScript", "setTimeout('window.scrollBy(0, -(document.body.clientHeight));', 1); ", True)
        Else
            Me.Visible = False
        End If
    End Sub
    ...
    Private Sub DisplayMessageDescBox(Optional ByVal strHeaderKey As String = "")
        ...
        Me.Visible = False
        If Not ViewState(VS_TEXTONLYMESSAGEBOX_CODETABLE) Is Nothing Then
            ViewState(VS_TEXTONLYMESSAGEDESCBOX) = True
            ViewState(VS_TEXTONLYMESSAGEBOX_SHOWED) = True
            ...
            If dtCode.Rows.Count > 0 Then
                ...
                For i = 0 To dtCode.Rows.Count - 1
                    ...
                    ElseIf strResult.Length > 1 Then
                        strMsgLang = strResult(0)
    ...
    Private Sub TextOnlyMessageBox_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRender
        If Not ViewState(VS_TEXTONLYMESSAGEDESCBOX) Is Nothing AndAlso CType(ViewState(VS_TEXTONLYMESSAGEDESCBOX), Boolean) Then
            '==================================================================== Code for SmartID ============================================================================
            Me.DisplayMessageDescBox()
            '==================================================================================================================================================================
        Else
            Me.DisplayMessageBox()
        End If
    End Sub
        ...
        Sub InstantiateIn(ByVal container As System.Web.UI.Control) _
            ...
            Select Case templateType
                ...
                Case DataControlRowType.DataRow
                    ...
                    'lcRowStart.Text = "<span style='font-weight:normal;font-style:normal;text-decoration:none;'><table border=0 cellspacing=0 cellpadding=0><tr><td width=1 valign='top'>"
                    'lblBullet.Text = "<span style='color:red;'>&bull;&nbsp;</span>"
                    'lc1.Text = "</td><td>"
                    lcRowStart.Text = "<span style='font-weight:normal;font-style:normal;text-decoration:none;font-size: 12pt; font-family: Arial'><table border=0 cellspacing=0 cellpadding=0><tr><td>"
                    lblMsgDesc.ForeColor = Drawing.Color.Red
                    lblMsgCode.ForeColor = Drawing.Color.Red
                    lcRowEnd.Text = "</td></table></span>"
                    ...
                    container.Controls.Add(lblBullet)
