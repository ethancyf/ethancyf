...
#Region "Variables / Constant"
    ...
    Private strFileNamePrefix As String = "CVP-EHS-MEX-"

    Private strVariableID_FileLastExportTime As String = "COVID19MEC_Record_Last_Export_DT"
    Private strParmName_FilePassword As String = "COVID19MECExportFilePassword"


    Public Const strErrorLogid As String = Common.Component.LogID.LOG00008

    Public Class COVID19MECExporterQueueStatus
...
#Region "Properties"
    ReadOnly Property Password() As String
        Get
            Me.m_udtCommonGeneralFunction.getSystemParameterPassword(strParmName_FilePassword, Me.m_strPassword)
    ...
    ReadOnly Property getCOVID19LastExportDt() As DateTime
        Get
            COVID19_Record_Last_Export_DT = Me.m_udtCommonGeneralFunction.GetSystemVariableValue(strVariableID_FileLastExportTime)
...
#End Region
    ...
    Public Sub StartCOVID19MECExporterProcess()
        Try
            C19Logger.Log(Common.Component.LogID.LOG00010, Nothing, "<Start><COVID19MECExporterProcess>", C19Logger.ScheduleJobLogAction.COVID19MECExporterProcess, ScheduleJobLogStatus.Information)
            ...
            C19Logger.Log(Common.Component.LogID.LOG00011, Nothing, "<Success><COVID19MECExporterProcess>", C19Logger.ScheduleJobLogAction.COVID19MECExporterProcess, ScheduleJobLogStatus.Success)

        Catch ex As Exception
            C19Logger.LogLine(ex.ToString())
            C19Logger.ErrorLog(ex)
            C19Logger.Log(strErrorLogid, Nothing, "<Error><COVID19MECExporterProcess>" & ex.ToString(), C19Logger.ScheduleJobLogAction.COVID19MECExporterProcess, ScheduleJobLogStatus.Fail)
        Finally
    ...
    Private Sub HandleRegenerateCases()
        Try
            ...
            objLogStartKeyStack.Push(C19Logger.Log(Common.Component.LogID.LOG00000, Nothing, "<Start><RegenerateMode>Start Process RegenerateCases Records...", C19Logger.ScheduleJobLogAction.ProcessRegenerateCase, ScheduleJobLogStatus.Information))
            'update all case with the file id m_strFileID to pending status
            udtC19BLL.UpdateC19QueueStatus(udtDB, m_strFileID, String.Empty, COVID19MECExporterQueueStatus.Pending)
            ExportAndZipFile()
            C19Logger.LogLine("[" + m_strFileID + "]Process RegenerateCases Records Finish")
            C19Logger.Log(Common.Component.LogID.LOG00000, objLogStartKeyStack.Pop, "<Success><RegenerateMode>Process RegenerateCases Records Finish...", C19Logger.ScheduleJobLogAction.ProcessRegenerateCase, ScheduleJobLogStatus.Success)
        Catch ex As Exception
            C19Logger.LogLine(ex.ToString())
            C19Logger.Log(strErrorLogid, objLogStartKeyStack.Pop, "<Error><RegenerateMode>" & ex.ToString(), C19Logger.ScheduleJobLogAction.ProcessRegenerateCase, ScheduleJobLogStatus.Fail)
            C19Logger.ErrorLog(ex)
        End Try
    End Sub
    ...
    Private Sub HandleFailCases()
        Try
            ...
            objLogStartKeyStack.Push(C19Logger.Log(Common.Component.LogID.LOG00000, Nothing, "<Start><chkFailRecord>Start Process Fail Records...", C19Logger.ScheduleJobLogAction.ProcessFailCase, ScheduleJobLogStatus.Information))
            'update fail cases to pending status
            udtC19BLL.UpdateC19QueueStatus(udtDB, String.Empty, COVID19MECExporterQueueStatus.Fail, COVID19MECExporterQueueStatus.Pending)
            ExportAndZipFile()
            C19Logger.LogLine("Process Fail Records Finish")
            C19Logger.Log(Common.Component.LogID.LOG00000, objLogStartKeyStack.Pop, "<Success><chkFailRecord>Process Fail Records Finish...", C19Logger.ScheduleJobLogAction.ProcessFailCase, ScheduleJobLogStatus.Success)

        Catch ex As Exception
            C19Logger.LogLine(ex.ToString())
            C19Logger.Log(strErrorLogid, objLogStartKeyStack.Pop, "<Error><chkFailRecord>" & ex.ToString(), C19Logger.ScheduleJobLogAction.ProcessFailCase, ScheduleJobLogStatus.Fail)
            C19Logger.ErrorLog(ex)
        End Try
    End Sub
    ...
    Private Sub HandleNormalCases()
        Try
            ...
            objLogStartKeyStack.Push(C19Logger.Log(Common.Component.LogID.LOG00000, Nothing, "<Start><chkNormalRecord>Start Process Normal Records...", C19Logger.ScheduleJobLogAction.ProcessNormalCase, ScheduleJobLogStatus.Information))
            ...
            If dtLastExport < dtCurrentDate Then
                Try
                    ...
                    objLogStartKeyStack.Push(C19Logger.Log(Common.Component.LogID.LOG00001, Nothing, "<Start><assignRecords>Start Check and Assign New Records...", C19Logger.ScheduleJobLogAction.AssignRecord, ScheduleJobLogStatus.Information))
                    ...
                    C19Logger.Log(Common.Component.LogID.LOG00001, objLogStartKeyStack.Pop, "<Success><assignRecords>Assign and Check New Records Success...", C19Logger.ScheduleJobLogAction.AssignRecord, ScheduleJobLogStatus.Success)
                Catch dbex As Exception
                    Try
                        udtDB.RollBackTranscation()
                    Catch rollbackex As Exception
                        C19Logger.Log(strErrorLogid, objLogStartKeyStack.Pop, "<Error><assignRecords>" & rollbackex.ToString(), C19Logger.ScheduleJobLogAction.AssignRecord, ScheduleJobLogStatus.Fail)
                        Throw rollbackex
                    End Try
            ...
            C19Logger.Log(Common.Component.LogID.LOG00000, objLogStartKeyStack.Pop, "<Success><chkNormalRecord>Process Normal Records Finish...", C19Logger.ScheduleJobLogAction.ProcessNormalCase, ScheduleJobLogStatus.Success)

        Catch ex As Exception
            C19Logger.LogLine(ex.ToString())
            C19Logger.Log(strErrorLogid, objLogStartKeyStack.Pop, "<Error><chkNormalRecord>" & ex.ToString(), C19Logger.ScheduleJobLogAction.ProcessNormalCase, ScheduleJobLogStatus.Fail)
            C19Logger.ErrorLog(ex)
        End Try
    End Sub
    ...
    Private Sub ExportAndZipFile()
        Try
            ...
            Do While True
                ...
                objLogStartKeyStack.Push(C19Logger.Log(Common.Component.LogID.LOG00002, Nothing, "<Start><chkPending>[" + strFileId + "]Check Pending Records...", C19Logger.ScheduleJobLogAction.LoadQueue, ScheduleJobLogStatus.Information))
                ...
                If dtC19Records.Rows.Count = 0 Then
                    ...
                    C19Logger.Log(Common.Component.LogID.LOG00002, objLogStartKeyStack.Pop, "<Success><chkPending>[" + strFileId + "]no any Pending record.", C19Logger.ScheduleJobLogAction.LoadQueue, ScheduleJobLogStatus.Success)
                    Exit Do
                Else
                    If m_strGenerateMode = GenerateMode.Regenerate Then
                        ...
                        C19Logger.Log(Common.Component.LogID.LOG00002, objLogStartKeyStack.Pop, "<Success><chkPending><RegenerateMode>[" + strFileId + "] Check Pending Records and A new file id is assigned ([" + m_strFileID + "] -> [" + strFileId + "])", C19Logger.ScheduleJobLogAction.LoadQueue, ScheduleJobLogStatus.Success)
                    Else
                        C19Logger.Log(Common.Component.LogID.LOG00002, objLogStartKeyStack.Pop, "<Success><chkPending>[" + strFileId + "]Check Pending Records...", C19Logger.ScheduleJobLogAction.LoadQueue, ScheduleJobLogStatus.Success)
                    End If
                ...
                Try
                    ...
                    objLogStartKeyStack.Push(C19Logger.Log(Common.Component.LogID.LOG00003, Nothing, "<Start><exportCSV>[" + strFileId + "]Export .csv File...", C19Logger.ScheduleJobLogAction.ExportCSV, ScheduleJobLogStatus.Information))
                    ...
                    If (System.IO.File.Exists(m_strExportFolderPath & strFileId & ".csv")) Then
                        ...
                        C19Logger.Log(Common.Component.LogID.LOG00003, objLogStartKeyStack.Pop, "<Success><exportCSV>[" + strFileId + "]Export .csv File...", C19Logger.ScheduleJobLogAction.ExportCSV, ScheduleJobLogStatus.Success)
                    Else
                        C19Logger.LogLine("[Error] CSV file(" & m_strExportFolderPath & strFileId & ".csv) cannot be found in export folder.")
                        C19Logger.Log(strErrorLogid, objLogStartKeyStack.Pop, "<Error><exportCSV>[" + strFileId + "] (" & m_strExportFolderPath & strFileId & ".csv) cannot be found in export folder.", C19Logger.ScheduleJobLogAction.ExportCSV, ScheduleJobLogStatus.Fail)
                    ...
                    objLogStartKeyStack.Push(C19Logger.Log(Common.Component.LogID.LOG00004, Nothing, "<Start><exportZip>[" + strFileId + "]Zip File...", C19Logger.ScheduleJobLogAction.ExportZIP, ScheduleJobLogStatus.Information))
                    ...
                    If (System.IO.File.Exists(m_strExportFolderPath & strFileId & ".zip")) Then
                        ...
                        C19Logger.Log(Common.Component.LogID.LOG00004, objLogStartKeyStack.Pop, "<Success><exportZip>[" + strFileId + "]Zip File...", C19Logger.ScheduleJobLogAction.ExportZIP, ScheduleJobLogStatus.Success)
                    Else
                        C19Logger.LogLine("[Error] Zip file(" & m_strExportFolderPath & strFileId & ".zip) cannot be found in export folder.")
                        C19Logger.Log(strErrorLogid, objLogStartKeyStack.Pop, "<Error><exportZip>[" + strFileId + "] (" & m_strExportFolderPath & strFileId & ".zip) cannot be found in export folder.", C19Logger.ScheduleJobLogAction.ExportZIP, ScheduleJobLogStatus.Fail)
                    ...
                    objLogStartKeyStack.Push(C19Logger.Log(Common.Component.LogID.LOG00005, Nothing, "<Start><exportCTL>[" + strFileId + "]Export .ctl File...", C19Logger.ScheduleJobLogAction.ExportCTL, ScheduleJobLogStatus.Information))
                    ...
                    If (System.IO.File.Exists(m_strExportFolderPath & strFileId & ".ctl")) Then
                        ...
                        C19Logger.Log(Common.Component.LogID.LOG00005, objLogStartKeyStack.Pop, "<Success><exportCTL>[" + strFileId + "]Export .ctl File...", C19Logger.ScheduleJobLogAction.ExportCTL, ScheduleJobLogStatus.Success)
                    Else
                        C19Logger.LogLine("[Error] CTL file(" & m_strExportFolderPath & strFileId & ".ctl) cannot be found in export folder.")
                        C19Logger.Log(strErrorLogid, objLogStartKeyStack.Pop, "<Error><exportCTL>[" + strFileId + "] (" & m_strExportFolderPath & strFileId & ".ctl) cannot be found in export folder.", C19Logger.ScheduleJobLogAction.ExportCTL, ScheduleJobLogStatus.Fail)
                    ...
                    objLogStartKeyStack.Push(C19Logger.Log(Common.Component.LogID.LOG00006, Nothing, "<Start><moveFile>[" + strFileId + "]Move File to Backup folder...", C19Logger.ScheduleJobLogAction.MoveFile, ScheduleJobLogStatus.Information))
                    ...
                    C19Logger.Log(Common.Component.LogID.LOG00006, objLogStartKeyStack.Pop, "<Success><moveFile>[" + strFileId + "]Move File to Backup folder...", C19Logger.ScheduleJobLogAction.MoveFile, ScheduleJobLogStatus.Success)

                    'update status to complete
                    C19Logger.LogLine("[" + strFileId + "] Strat Update Status to Complete By File_id ")
                    objLogStartKeyStack.Push(C19Logger.Log(Common.Component.LogID.LOG00007, Nothing, "<Start><updateStatus>[" + strFileId + "]Update Status to Complete By File_id...", C19Logger.ScheduleJobLogAction.ExportAndZipFile, ScheduleJobLogStatus.Information))
                    udtC19BLL.UpdateC19QueueStatus(udtDB, strFileId, COVID19MECExporterQueueStatus.Processing, COVID19MECExporterQueueStatus.Complete)
                    C19Logger.LogLine("[" + strFileId + "] Update Status to Complete By File_id Success")
                    C19Logger.Log(Common.Component.LogID.LOG00007, objLogStartKeyStack.Pop, "<Success><updateStatus>[" + strFileId + "]Update Status to Complete By File_id...", C19Logger.ScheduleJobLogAction.ExportAndZipFile, ScheduleJobLogStatus.Success)
                ...
                Catch ex As Exception
                    ...
                    C19Logger.LogLine(ex.ToString())
                    C19Logger.Log(strErrorLogid, objLogStartKeyStack.Peek, "<Error>[" + strFileId + "]Export File Fail..." + ex.ToString(), C19Logger.ScheduleJobLogAction.ExportAndZipFile, ScheduleJobLogStatus.Fail)
                    C19Logger.ErrorLog(ex)
                    udtC19BLL.UpdateC19QueueStatus(udtDB, strFileId, COVID19MECExporterQueueStatus.Processing, COVID19MECExporterQueueStatus.Fail)
    ...
    Private Sub updateCOVID19RecordLastExportDT(ByVal LastImportName As String, ByRef udtDB As Common.DataAccess.Database)
        Try
            Me.m_udtCommonGeneralFunction.UpdateSystemVariable(strVariableID_FileLastExportTime, LastImportName, "eHS", Nothing, udtDB)
        Catch ex As Exception
            Throw
        End Try
    End Sub
