...
#Region "Internal Class"
        ...
        <Serializable()> Public Class HandleMethodClass
            ...
            Public Const WarningReason = "WarningReason"
            Public Const Exception = "Exception"
            Public Const Block = "Block"
            Public Const BlockPopup = "BlockPopup"
...
#End Region
            ...
            Sub New(ByVal strSchemeCode As String, ByVal intSchemeSeq As Integer, ByVal strSubsidizeCode As String, ByVal strRuleGroupCode As String, _
                ...
                Select Case strHandleMethod
                    ...
                    Case HandleMethodClass.Warning
                        ...
                        Me._blnEligible = True
                    Case HandleMethodClass.WarningReason
                        Me._enumHandleMethod = HandleMethodENum.WarningReason
                        Me._blnEligible = True
                    ...
                    Case HandleMethodClass.Block
                        ...
                        Me._blnEligible = False
                    Case HandleMethodClass.BlockPopup
                        Me._enumHandleMethod = HandleMethodENum.BlockPopup
                        Me._blnEligible = False
        ...
        <Serializable()> Class ClaimRuleResult
            ...
            Sub New(ByVal udtClaimRuleResult As ClaimRuleResult)
                Me._strSchemeCode = udtClaimRuleResult.SchemeCode
                Me._intSchemeSeq = udtClaimRuleResult.SchemeSeq
                Me._strSubsidizeCode = udtClaimRuleResult.SubsidizeCode
                Me._strRuleGroupCode = udtClaimRuleResult.RuleGroupCode

                Me._blnMatchedCase = udtClaimRuleResult.IsMatched

                Me._strHandleMethod = udtClaimRuleResult.HandleMethod
                Me._dicResultParam = New Dictionary(Of String, Object)

                For Each udtKVP As KeyValuePair(Of String, Object) In udtClaimRuleResult.ResultParam
                    Me._dicResultParam.Add(udtKVP.Key, udtKVP.Value)
                Next

                Me._enumRuleType = udtClaimRuleResult.RuleType

                Me._blnBlock = udtClaimRuleResult.IsBlock
                Me._enumHandleMethod = udtClaimRuleResult.HandleMethod
                        ...
                        For Each udtTranDetail As TransactionDetailModel In udtEHSTransaction.TransactionDetails
                            If strMsgCode = "" Then
                                ...
                                If udtTranDetail.SubsidizeItemCode.Trim = SubsidizeGroupClaimModel.SubsidizeItemCodeClass.C19 AndAlso _
                                    udtTranDetail.AvailableItemCode.Trim <> SubsidizeItemDetailsModel.DoseCode.FirstDOSE Then
                ...
                If udtDoseRuleResult.IsMatch Then
                    ...
                    If intTranBenefitCount < udtSubsidizeItemDetail.AvailableItemNum Then
                        ...
                        For Each udtEqvSubsidizeMapModel As EqvSubsidizeMapModel In udtEqvSubsidizeMapList
                            ...
                            If udtFilteredTranBenefitList IsNot Nothing AndAlso udtFilteredTranBenefitList.Count > 0 Then
                                'Update count
...
#Region "Claim Rules"
            ...
            Select Case udtClaimRule.Type.Trim().ToUpper()
                ...
                Case ClaimRuleModel.RuleTypeClass.CHECK_ON_LIST
                    ...
                    Select Case udtClaimRule.CompareValue.Trim().ToUpper()
                        ...
                        Case CheckList.COVID19InfectedDischargeList
                ...
                Case ClaimRuleModel.RuleTypeClass.VACCINE_BRAND
                    ...
                    If udtClaimRule.Target.Trim().ToUpper() = strDoseCode.Trim().ToUpper() Then
                        ...
                        'For Each udtVC As TransactionDetailVaccineModel In udtVaccinationRecordList
                        '    If udtVC.AvailableItemCode = SubsidizeItemDetailsModel.DoseCode.FirstDOSE Then
                        '        udtVaccinationRecord = udtVC
                        '    End If
                        'Next

                        udtNearestVaccinationRecord = udtVaccinationRecordList.FilterFindNearestRecord(dtmServiceDate)

                        If udtNearestVaccinationRecord Is Nothing Then Return False

                        'If recipient on discharge list and has injected 1st dose before infected, bypass the checking
                        Dim udtDischargeResult As DischargeResultModel = udtInputPicker.DischargeResult
                        ...
                        If udtDischargeResult IsNot Nothing AndAlso _
                            ...
                            If strDoseCode <> SubsidizeItemDetailsModel.DoseCode.FirstDOSE AndAlso _
                                ...
                                dtmNearestDoseInject = udtNearestVaccinationRecord.ServiceReceiveDtm
                                'End If

                                'Case 1: 1STDOSE < Discharge Date
                        ...
                        For Each udtVC As TransactionDetailVaccineModel In udtVaccinationRecordList
                            If udtVC.AvailableItemCode.ToUpper.Trim = udtClaimRule.Dependence.ToUpper.Trim Then
                                If RuleComparator(udtClaimRule.Operator, udtInputPicker.Brand.ToUpper.Trim, udtVC.VaccineBrand.ToUpper.Trim) Then
                                    ' Error
                                    Return True
                                    Exit For
                                End If
                ...
                Case ClaimRuleModel.RuleTypeClass.VACCINE_WINDOW
                    ...
                    If udtClaimRule.Target.Trim().ToUpper() = strDoseCode.Trim().ToUpper() Then
                        ...
                        Dim udtNearestVaccinationRecord As TransactionDetailVaccineModel = udtVaccinationRecordList.FilterFindNearestRecord(dtmServiceDate)

                        If udtNearestVaccinationRecord Is Nothing Then Return False

                        If udtClaimRule.Dependence.Trim().ToUpper() <> udtNearestVaccinationRecord.AvailableItemCode.Trim.Trim().ToUpper() Then Return False
                        ...
                        If dr.Length > 0 Then
                            ...
                            If udtDischargeResult IsNot Nothing AndAlso _
                                ...
                                If strDoseCode <> SubsidizeItemDetailsModel.DoseCode.FirstDOSE AndAlso _
                                    ...
                                    'Case 1: 1STDOSE < Discharge Date
                                    If strCompareValue <> String.Empty AndAlso dtmDischarge IsNot Nothing AndAlso dtmNearestDoseInject IsNot Nothing Then
                                        If dtmNearestDoseInject < dtmDischarge Then
                                            'Not need to handle
                                            Return False
                                        End If
                            ...
                            Dim blnResult As Boolean = CheckClaimRuleSingleEntryByInnerDose(dtmServiceDate, _
                                                                                            udtNearestVaccinationRecord.ServiceReceiveDtm, _
                                                                                            udtClaimRule, _
                                                                                            dicResultParam)
                                ...
                                If Not dicResultParam.ContainsKey("%DaysApart") Then
                                    ...
                                    Else
                                        If (Int(udtClaimRule.CompareValue) <> 0 AndAlso udtClaimRule.Operator = "<") OrElse _
                                            ...
                                            dicResultParam.Add("%DaysApart", CStr(Math.Abs(Int(udtClaimRule.CompareValue))))
                                        End If

                                        If (Int(udtClaimRule.CompareValue) <> 0 AndAlso udtClaimRule.Operator = "<=") OrElse _
                                            (Int(udtClaimRule.CompareValue) <> 0 AndAlso udtClaimRule.Operator = ">=") Then
                                            dicResultParam.Add("%DaysApart", CStr(Math.Abs(Int(udtClaimRule.CompareValue)) + 1))
                                        End If
                                    End If
                                End If
                ...
                Case ClaimRuleModel.RuleTypeClass.DISCHARGE
                    If strDoseCode.Trim().ToUpper() = String.Empty Then Return False
                    ...
                    If udtClaimRule.Target.Trim().ToUpper() = strDoseCode.Trim().ToUpper() Then
                        ...
                        If blnOnList Then
                            ...
                            'Common
                            Dim strCompareDischargeValue As String = String.Empty
                            Dim strCompareWindowValue As String = String.Empty
                            Dim dtmDischarge As Nullable(Of DateTime) = Nothing

                            Dim udtNearestVaccinationRecord As TransactionDetailVaccineModel = Nothing
                            Dim udtNearestVaccinationRecordByServiceDate As TransactionDetailVaccineModel = Nothing
                            Dim udtVaccinationRecordList As TransactionDetailVaccineModelCollection = Nothing
                            Dim dtmNearestDoseInject As Nullable(Of DateTime) = Nothing
                            ...
                            If dr.Length > 0 Then
                                strCompareDischargeValue = dr(0)("Discharge_Window_Min").ToString.Trim
                                strCompareWindowValue = dr(0)("Vaccination_Window_Min").ToString.Trim
                            ...
                            If strDoseCode <> SubsidizeItemDetailsModel.DoseCode.FirstDOSE AndAlso _
                                ...
                                If udtVaccinationRecordList IsNot Nothing AndAlso udtVaccinationRecordList.Count > 0 Then
                                    ...
                                    '        udtVaccinationRecord = udtVR
                                    '        dtm1stDoseInject = udtVR.ServiceReceiveDtm
                                    '        Exit For
                                    '    End If
                                    'Next
                                    ...
                                    udtNearestVaccinationRecordByServiceDate = udtVaccinationRecordList.FilterFindNearestRecord(dtmServiceDate)

                                    If udtNearestVaccinationRecordByServiceDate IsNot Nothing Then
                                        dtmNearestDoseInject = udtNearestVaccinationRecordByServiceDate.ServiceReceiveDtm
                                    End If
                            ...
                            ''1. Check brand, not allow CoronaVac
                            '    If udtInputPicker.Brand.ToUpper.Trim = 1 Then ' 1 = CoronaVac
                            '        blnValid = False
                            ...
                            If strDoseCode <> SubsidizeItemDetailsModel.DoseCode.FirstDOSE AndAlso _
                                ...
                                If dtmDischarge IsNot Nothing AndAlso dtmNearestDoseInject IsNot Nothing Then
                                    If dtmNearestDoseInject <= dtmDischarge Then
                                        ...
                                        If udtVaccinationRecordList IsNot Nothing AndAlso udtVaccinationRecordList.Count > 0 Then
                                            For Each udtVR As TransactionDetailVaccineModel In udtVaccinationRecordList
                                                If udtInputPicker.Brand.ToUpper.Trim <> udtVR.VaccineBrand.ToUpper.Trim Then
                                                    blnValid = False
                                                    Exit For
                                                End If
                                            Next
                                        ...
                                        If Not blnValid Then
                                            lstSystemMessage.Add(New ComObject.SystemMessage(FunctCode.FUNT990000, SeverityCode.SEVE, MsgCode.MSG00486))
                                        End If
                            ...
                            If strDoseCode <> SubsidizeItemDetailsModel.DoseCode.FirstDOSE AndAlso _
                                ...
                                '        If Math.Abs(DateDiff(DateInterval.Day, CDate(udtDischargeResult.DischargeDate), udtNearestVaccinationRecord.ServiceReceiveDtm)) >= 0 Then
                                '            blnValid = False
                ...
                Case ClaimRuleModel.RuleTypeClass.NON_LOCAL_RECOVERED
                    ...
                    If udtClaimRule.Target.Trim().ToUpper() = strDoseCode.Trim().ToUpper() Then
                        ...
                        If blnHasHistory Then
                            ...
                            ''When claims 2nd / 3rd dose, find the latest C19 vaccination record
                            'If strDoseCode = SubsidizeItemDetailsModel.DoseCode.SecondDOSE OrElse _
                            '   strDoseCode = SubsidizeItemDetailsModel.DoseCode.ThirdDOSE Then

                            '    'Find the all C19 vaccination record
                            '    udtVaccinationRecordList = udtInputPicker.VaccinationRecord.FilterIncludeBySubsidizeItemCode(SubsidizeGroupClaimModel.SubsidizeItemCodeClass.C19)
                        ...
                        Return CheckClaimRuleSingleEntryByCount(IIf(blnHasHistory, 1, 0), udtClaimRule.Operator.Trim())
        ...
        ' ---------------------------------------------------------------------------------------------------------
        Private Shared Function CheckClaimRuleSingleEntryByCount(ByVal intCount As Integer, ByVal strOperator As String) As Boolean
            Dim blnRes As Boolean = False
            ...
            If dicResultParam IsNot Nothing AndAlso udtLatestTransactionDetail IsNot Nothing Then ' Nothing if checking SubsidizeItemDetailRule
                ...
                If (Int(strCompareValue) <> 0 AndAlso strOperator = "<=") OrElse (Int(strCompareValue) <> 0 AndAlso strOperator = ">=") OrElse _
                    ...
                    If (Int(strCompareValue) <> 0 AndAlso strOperator = "<=") OrElse (Int(strCompareValue) <> 0 AndAlso strOperator = ">=") Then
                        dicResultParam.Add("%DaysApart", CStr(Math.Abs(Int(strCompareValue)) + 1))
                    End If

                    If (Int(strCompareValue) <> 0 AndAlso strOperator = "<") OrElse (Int(strCompareValue) <> 0 AndAlso strOperator = ">") Then
                        dicResultParam.Add("%DaysApart", CStr(Math.Abs(Int(strCompareValue))))
                    End If
                End If
            End If
