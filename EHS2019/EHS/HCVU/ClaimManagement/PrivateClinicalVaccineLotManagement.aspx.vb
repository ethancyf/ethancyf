...
Partial Public Class PrivateClinicalVaccineLotManagement
...
#Region "Fields"
    ...
    Private udtSchemeBackOfficeBLL As New Scheme.SchemeBackOfficeBLL
    Private udtSchemeBackOfficeModel As Scheme.SchemeBackOfficeModel
    'new
...
#Region "Page Events"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ' Get valid schemes   ' no need practice????
        ...
        If Not Page.IsPostBack Then
            FunctionCode = FunctCode.FUNT010427

            ' Write Audit Log
            udtAuditLogEntry = New AuditLogEntry(FunctionCode, Me)
            udtAuditLogEntry.WriteLog(LogID.LOG00000, AuditMsg00000)
            'BindCentre(ddlVaccScheme)                                   //need change this func
            'DropDownListBindScheme(ddlVaccScheme, udtSchemeClaimList, _udtSessionHandler.Language, True) //change
            BindScheme(ddlSearchVaccScheme)
            ddlSearchVaccScheme.Visible = True

            If ddlSearchVaccScheme.Items.Count > 1 OrElse ddlSearchVaccScheme.Items.Count = 0 Then
                btnAssign.Enabled = False
                'btnBatchRemove.Enabled = False

                btnAssign.ImageUrl = GetGlobalResourceObject("ImageUrl", "AssignDisableBtn")
...
#End Region
    ...
    Private Sub GetVaccineLotSummary()
        udtAuditLogEntry = New AuditLogEntry(FunctionCode, Me)
        'udtAuditLogEntry.AddDescripton("Centre Name", ddlVaccScheme.SelectedItem.ToString)
        'udtAuditLogEntry.AddDescripton("Centre Id", ddlVaccScheme.SelectedValue.ToString)
        udtAuditLogEntry.WriteStartLog(LogID.LOG00002, AuditMsg00002)
        Try
            Dim VaccineLotSummaryDataTable As DataTable = udtVaccineLotBLL.GetVaccineLotListSummaryByPrivateClinic(VaccineLotRecordServiceType.Scheme, ddlSearchVaccScheme.SelectedValue)

            Session(SESS_VaccineLotSummary) = VaccineLotSummaryDataTable

            GridViewDataBind(gvSummary, VaccineLotSummaryDataTable, "Brand_Name", "ASC", False) ' need change
    ...
    'new -- Scheme DropDownList
    Private Sub BindScheme(ByVal ddlSearchVaccScheme As DropDownList)
        ...
        'Session(SESS_SchemeClaimList) = udtSchemeCList  '//use for cache
                ...
                For intCt As Integer = 0 To strSchemeCode.Length - 1
                    ...
                    For Each udtUserRoleModel As UserRoleModel In udtUserRoleCollection.Values
                        If udtUserRoleModel.SchemeCode.Trim = udtSchemeClaim.SchemeCode Then
                            If Not udtSchemeClaimModelListFilter.Contains(udtSchemeClaim) Then udtSchemeClaimModelListFilter.Add(udtSchemeClaim) '(udtSchemeClaim.SchemeDesc.ToString())
                        End If
        ...
        'Session(SESS_SchemeClaimListFilteredByUserRole) = udtSchemeClaimModelListFilter //use for cache

        ddlSearchVaccScheme.DataSource = udtSchemeClaimModelListFilter
        ddlSearchVaccScheme.DataValueField = "SchemeCode"
        ddlSearchVaccScheme.DataTextField = "SchemeDesc"
        ddlSearchVaccScheme.DataBind()

        ' Set the scheme list to disabled if only 1 scheme
        If udtSchemeClaimModelListFilter.Count = 1 Then
            ddlSearchVaccScheme.SelectedIndex = 1
            ddlSearchVaccScheme.Enabled = False  ' Here use Batch Assign image

        ElseIf udtSchemeClaimModelListFilter.Count = 0 Then
            ...
            ddlSearchVaccScheme.Enabled = False

        End If
        'If no. of items is more than 1, then add "Please Select" at the top of dropdownlist
        If ddlSearchVaccScheme.Items.Count > 1 Then
            ddlSearchVaccScheme.Items.Insert(0, New ListItem(Me.GetGlobalResourceObject("Text", "PleaseSelect"), String.Empty))
            ddlSearchVaccScheme.SelectedIndex = 0
    ...
    Protected Sub ibtnMsgBack_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        ...
        If ddlSearchVaccScheme.SelectedValue <> String.Empty Then
            GetVaccineLotSummary()
        End If
    ...
    Protected Sub ibtnAssign_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        udtAuditLogEntry = New AuditLogEntry(FunctionCode, Me)
    ...
    Protected Sub ddlSearchVaccScheme_SelectedIndexChanged(sender As Object, e As EventArgs) '//change here
        udtAuditLogEntry = New AuditLogEntry(FunctionCode, Me)
        ...
        If ddlSearchVaccScheme.SelectedValue = String.Empty Then
            ...
            dt = Nothing
            GridViewDataBind(gvSummary, dt, "Brand_Name", "Asc", False) 'change?
            btnAssign.Enabled = False
            'btnBatchRemove.Enabled = False
            btnAssign.ImageUrl = GetGlobalResourceObject("ImageUrl", "AssignDisableBtn")
        ...
        Else
            ...
            btnAssign.ImageUrl = GetGlobalResourceObject("ImageUrl", "AssignBtn")
            'dr = dtVaccinCentreSP.Select(String.Format("Scheme = '{0}'", ddlVaccScheme.SelectedValue)) 'change
            GetVaccineLotSummary()
            Dim dtResult As DataTable = New DataTable
            dtResult = CType(Session(SESS_VaccineLotSummary), DataTable)
            'Dim dtVaccineLotSummaryDataTable As DataTable = udtVaccineLotBLL.GetVaccineLotListSummaryByPrivateClinic(VaccineLotRecordServiceType.Scheme, ddlSearchVaccScheme.SelectedValue)
...
#Region "Common Function"
    ...
    Protected Sub ddlVaccineScheme_SelectedIndexChanged(sender As Object, e As EventArgs)  'change to  be scheme
        'If (ddlVaccineScheme.SelectedValue <> String.Empty) Then
        '    ddlvaccinecentre(Me.lbVaccineCentreBooth, ddlvaccinecentre.SelectedValue)  'change, can be deleted?
        'Else
    ...
    Private Sub BuildBrand(ByVal ddl As DropDownList, ByVal blnIsAny As Boolean)
        ...
        Select Case ViewState("state")
            ...
            Case StateType.BatchRemove
                dtAllBrand = udtVaccineLotBLL.GetCOVID19VaccineBrandDetailByCentre(ddlSearchVaccScheme.SelectedItem.Value.Trim)
    ...
    Private Sub BuildVaccineLotNo(ByVal ddl As DropDownList, ByVal strBrandId As String)
        ...
        Select Case ViewState("state")
            ...
            Case StateType.BatchRemove
                ...
                Else
                    dtAllVaccine = udtVaccineLotBLL.GetCOVID19VaccineBrandDetailByCentre(ddlSearchVaccScheme.SelectedItem.Value.Trim)
                End If
                dvVaccineLotNo = New DataView(dtAllVaccine)
    ...
    Private Sub InitializeDataValue()
        ...
        'lblInputVaccineScheme.Items.Insert(0, New ListItem(ddlVaccScheme.SelectedItem.Text.Trim, ddlVaccScheme.SelectedItem.Value.Trim)) '//need change
        'ddlVaccineScheme.SelectedIndex = 0
        'ddlVaccineScheme.Enabled = True

        'If (ddlSearchVaccScheme.SelectedItem.Value.ToString = "VSS") Then
        '    ddlInputVaccineScheme.Text = VaccineLotRecordClinics.VSS
        'ElseIf ddlSearchVaccScheme.SelectedItem.Value.ToString = "RVP" Then
        '    ddlInputVaccineScheme.Text = VaccineLotRecordClinics.RVP
        'ElseIf ddlSearchVaccScheme.SelectedItem.Value.ToString = "COVID19DH" Then
        '    ddlInputVaccineScheme.Text = VaccineLotRecordClinics.COVID19DH
        'End If
        ...
        udtSchemeBackOfficeModel = udtSchemeBackOfficeBLL.GetAllSchemeBackOfficeWithSubsidizeGroup().Filter(ddlSearchVaccScheme.SelectedItem.Value.ToString)
        SchemeName = udtSchemeBackOfficeModel.SchemeDesc.Trim
        lblConVaccineScheme.Text = SchemeName

        ddlInputVaccineScheme.Text = SchemeName
    ...
    Private Sub ResetAllInputPageBind()
        'ddlVaccScheme.Items.Clear()  'need or not??
...
#Region "Vaccine Lot Record Event"
    ...
    Private Sub GetVaccineLotForConfirmPage()
        ...
        Select Case ViewState("state")
            Case StateType.ADD, StateType.EDIT
                gvConfirmDetail.Columns(2).Visible = False
            Case StateType.BatchRemove
                gvConfirmDetail.Columns(2).Visible = True
        End Select
    ...
    Protected Sub ibtnRecordSave_Click(sender As Object, e As ImageClickEventArgs)
        ...
        Try
            ...
            If ViewState("state") = StateType.ADD Then
                ...
                udtAuditLogEntry.AddDescripton("Scheme", ddlSearchVaccScheme.SelectedValue.ToString)
            ...
            ElseIf ViewState("state") = StateType.EDIT Then
                'udtAuditLogEntry.AddDescripton("Scheme", hfEVaccineLotId.Text)
            ...
            If (SaveRecordInfoValidation()) Then
                ...
                If ViewState("state") = StateType.ADD Then
                    ...
                    SchemeName = udtSchemeBackOfficeModel.SchemeDesc.Trim
                    lblConVaccineScheme.Text = SchemeName
                    'lblConVaccineScheme.Text = ddlSearchVaccScheme.SelectedItem.ToString
                    ...
                    'hfConVaccineCentreId.Text = ddlSearchVaccScheme.SelectedValue.ToString
                ...
                ElseIf ViewState("state") = StateType.EDIT Then
                    ...
                    udtVaccineLotList = udtVaccineLotBLL.GetVaccineLotModalByVaccineLotId(hfEVaccineLotId.Text, VaccineLotRecordServiceType.Scheme)
                    Dim udtSchemeBackOfficeBLL As New Scheme.SchemeBackOfficeBLL
                    Dim udtSchemeBackOfficeModel As Scheme.SchemeBackOfficeModel
                    Dim SchemeName As String = String.Empty
                    udtSchemeBackOfficeModel = udtSchemeBackOfficeBLL.GetAllSchemeBackOfficeWithSubsidizeGroup().Filter(ddlSearchVaccScheme.SelectedItem.Value.ToString)
                    SchemeName = udtSchemeBackOfficeModel.SchemeDesc.Trim
                    lblConVaccineScheme.Text = SchemeName
                    hflblEConVaccineSchemeCode.Text = udtVaccineLotList.SchemeCode
                    'lblConVaccineScheme.Text = udtVaccineLotList.SchemeCode
                    ...
                    End If
                    dtconfirmPageDisplay.Rows.Add(DateTime.Parse(udtVaccineLotList.VaccineLotEffectiveDFrom), strTempEffectiveDtm, udtVaccineLotList.UpToExpiryDtm, udtVaccineLotList.RecordStatus)
    ...
    Protected Sub ibtnRecordConfirm_Click(sender As Object, e As ImageClickEventArgs)
        ...
        If ViewState("state") = StateType.ADD Then
            ...
            If (blnValid) Then
                ...
                If dvLotDetailFilterWithRecordStatus.Count > 0 Then
                    msgBox.AddMessage(New Common.ComObject.SystemMessage(FunctionCode, SeverityCode.SEVE, MsgCode.MSG00008), "%s", lblConVaccineLotNo.Text.Trim)
                    blnValid = False
                End If
            End If
        ...
        Try
            If (blnValid) Then
                ...
                ibtnRecordConfirm.Enabled = True
                ibtnRecordConfirm.ImageUrl = GetGlobalResourceObject("ImageUrl", "ConfirmBtn")
                'udtVaccineLotList.ServiceType = lblConVaccineScheme.Text.Trim
                udtVaccineLotList.ServiceType = VaccineLotRecordServiceType.Scheme
                udtVaccineLotList.SchemeCode = ddlSearchVaccScheme.SelectedItem.Value.ToString
                ...
                udtAuditLogEntry.AddDescripton("Scheme", udtVaccineLotList.ServiceType)
                ...
                ElseIf ViewState("state") = StateType.EDIT Then
                    'udtVaccineLotList.ServiceType = lblConVaccineScheme.Text.Trim
                    udtVaccineLotList.ServiceType = VaccineLotRecordServiceType.Scheme
                    udtVaccineLotList.SchemeCode = ddlSearchVaccScheme.SelectedItem.Value.ToString  'change??
                    ...
                    udtAuditLogEntry.AddDescripton("Scheme", udtVaccineLotList.SchemeCode)
    ...
    Protected Sub AddSingleRemoveRecordRequest()
        ...
        'If (lblEConVaccineScheme.Text = VaccineLotRecordClinics.VSS) Then
        '    SchemeCode = VaccineLotRecordSchemeCode.VSS
        'ElseIf lblEConVaccineScheme.Text = VaccineLotRecordClinics.RVP Then
        '    SchemeCode = VaccineLotRecordSchemeCode.RVP
        'ElseIf lblEConVaccineScheme.Text = VaccineLotRecordClinics.COVID19DH Then
        '    SchemeCode = VaccineLotRecordSchemeCode.COVID19DH
        'End If
        ...
        Try
            ...
            If RemoveRecordInfoValidation(VaccineLotRecordServiceType.Scheme, hflblEConVaccineSchemeCode.Text, lblEConVaccineLotNo.Text) Then  '//need change
                Dim udtVaccineLotList As VaccineLotModel = New VaccineLotModel()
                'udtVaccineLotList.CentreId = hflblEConVaccineCentreId.Text
                'udtVaccineLotList.BoothId = hflEConVaccineBoothId.Text
                udtVaccineLotList.ServiceType = VaccineLotRecordServiceType.Scheme
                udtVaccineLotList.SchemeCode = hflblEConVaccineSchemeCode.Text
                'udtVaccineLotList.SchemeCode = lblEConVaccineScheme.Text
...
#End Region
    ...
    Private Sub bindEditView()
        ...
        ElseIf ViewState("state") = StateType.EDIT Then
            ...
            'If (udtVaccineLotList.SchemeCode = "VSS") Then
            '    lblPERConVaccineScheme.Text = VaccineLotRecordClinics.VSS
            'ElseIf udtVaccineLotList.SchemeCode = "RVP" Then
            '    lblPERConVaccineScheme.Text = VaccineLotRecordClinics.RVP
            'ElseIf udtVaccineLotList.SchemeCode = "COVID19DH" Then
            '    lblPERConVaccineScheme.Text = VaccineLotRecordClinics.COVID19DH
            'End If
            ...
            udtSchemeBackOfficeModel = udtSchemeBackOfficeBLL.GetAllSchemeBackOfficeWithSubsidizeGroup().Filter(ddlSearchVaccScheme.SelectedItem.Value.ToString)
            SchemeName = udtSchemeBackOfficeModel.SchemeDesc.Trim
            lblPERConVaccineScheme.Text = SchemeName
            'lblPERConVaccineScheme.Text = udtVaccineLotList.SchemeCode
    ...
    Private Sub ChangeViewTitle(ByVal viewTitle As Label)
        ...
        Select Case ViewState("state")
            Case StateType.ADD
                viewTitle.Text = Me.GetGlobalResourceObject("Text", "VaccineLotRecordAssign")
                'Case StateType.BatchRemove
    ...
    Protected Sub BindVaccineInfoView(ByVal strVLID As String)
        ...
        udtVaccineLotList = udtVaccineLotBLL.GetVaccineLotModalByVaccineLotId(strVLID, VaccineLotRecordServiceType.Scheme)
        ...
        ChangeViewIndex(ViewIndex.EditRecordView)
        'If (udtVaccineLotList.SchemeCode = "VSS") Then
        '    lblEConVaccineScheme.Text = VaccineLotRecordClinics.VSS
        'ElseIf udtVaccineLotList.SchemeCode = "RVP" Then
        '    lblEConVaccineScheme.Text = VaccineLotRecordClinics.RVP
        'ElseIf udtVaccineLotList.SchemeCode = "COVID19DH" Then
        '    lblEConVaccineScheme.Text = VaccineLotRecordClinics.COVID19DH
        'End If

        lblEConVaccineScheme.Text = SchemeName
        'lblEConVaccineScheme.Text = udtVaccineLotList.SchemeCode
        hflblEConVaccineSchemeCode.Text = udtVaccineLotList.SchemeCode
...
#Region "Validation"
    ...
    Private Function RemoveRecordInfoValidation(ByVal strServiceType As String, ByVal strSchemeCode As String, ByVal strVaccLotNo As String) As Boolean
        Dim blnValid As Boolean = True
        Dim blnRequestPendingList As Boolean = False
        Dim blnNonExistRecord As Boolean = False

        'VaccineLotSummaryDataTable = udtVaccineLotBLL.GetVaccineLotListSummaryByPrivateClinic(strScheme)

        'check active record
        blnNonExistRecord = ckNonExistRecord(strServiceType, strSchemeCode, strVaccLotNo)
        'Dim dtVaccineLotExistLot As DataTable = Nothing
        'dtVaccineLotExistLot = udtVaccineLotBLL.GetVaccineLotExistByServiceType(strServiceType, strSchemeCode, strVaccLotNo)

        If blnNonExistRecord = False Then 'dtVaccineLotExistLot.Rows.Count = 0
            msgBox.AddMessage(New Common.ComObject.SystemMessage(FunctionCode, SeverityCode.SEVE, MsgCode.MSG00010)) ' change 
            blnValid = False
        ...
        If blnValid Then
            ...
            blnRequestPendingList = ckPendingRequestRecordExist(strServiceType, strSchemeCode, strVaccLotNo)

            If blnRequestPendingList = True Then
                msgBox.AddMessage(New Common.ComObject.SystemMessage(FunctionCode, SeverityCode.SEVE, MsgCode.MSG00004)) 'boolean >> true > go , false > msg004
                blnValid = False
            End If
        End If
        Return blnValid
    ...
    Private Function SaveRecordInfoValidation() As Boolean
        ...
        If (ViewState("state") = StateType.ADD Or ViewState("state") = StateType.EDIT) Then
            ...
            If blnValid Then
                ...
                End If
                Dim blnVaccineLotDuplicated As Boolean = False
                If ViewState("state") = StateType.ADD Then
                    ...
                    blnVaccineLotDuplicated = ckDuplicatedEffectiveDate(VaccineLotRecordServiceType.Scheme, ddlSearchVaccScheme.SelectedValue.ToString, ddlVaccineLotNo.SelectedValue.ToString, txtVaccineLotEffectiveDateFrom.Text.Trim, chkeffectiveDateTo)
                ElseIf ViewState("state") = StateType.EDIT Then
                    If hfVaccineLotEffectiveDateFrom.Text = txtVaccineLotEffectiveDateFrom.Text.Trim Then
                        ...
                        If (hfVaccineLotEffectiveDateTo.Text = txtVaccineLotEffectiveDateTo.Text.Trim Or _
                            ...
                            blnVaccineLotDuplicated = True
                        End If
                    End If
                End If

                If blnVaccineLotDuplicated = True Then
                    msgBox.AddMessage(New Common.ComObject.SystemMessage(FunctionCode, SeverityCode.SEVE, MsgCode.MSG00007), {"%ef", "%et"}, {GetGlobalResourceObject("Text", "VaccineLotEffectiveDateFrom"), GetGlobalResourceObject("Text", "VaccineLotEffectiveDateTo")})
                    blnValid = False
                End If
            End If
            ...
            If blnValid Then
                ' Dim VaccineLotSummaryDataTable As DataTable = Nothing
                Dim blnRequestPendingListRecordExist As Boolean = False
                If ViewState("state") = StateType.ADD Then
                    ...
                    blnRequestPendingListRecordExist = ckPendingRequestRecordExist(VaccineLotRecordServiceType.Scheme, ddlSearchVaccScheme.SelectedValue.ToString, ddlVaccineLotNo.SelectedValue.ToString)
                ElseIf ViewState("state") = StateType.EDIT Then
                    ...
                    blnRequestPendingListRecordExist = ckPendingRequestRecordExist(VaccineLotRecordServiceType.Scheme, ddlSearchVaccScheme.SelectedItem.Value.ToString, lblPERConVaccineLotNo.Text)
                End If
                If blnRequestPendingListRecordExist = True Then
    ...
    Private Function CheckPastDate(ByRef DateText As TextBox, ByRef lblErrMsg As Label, ByRef ErrImage As Image, ByRef UpToExpiryDate As Boolean) As Boolean
        ...
        Else
            ...
            If dtmDate < DateTime.Today.Date Then
                msgBox.AddMessage(FunctionCode, SeverityCode.SEVE, MsgCode.MSG00005) 'System Effective To Date" cannot be a past date. (only to date)
                ErrImage.Visible = True
                Return False
            End If
    ...
    Private Function ckPendingRequestRecordExist(ByVal strServiceType As String, ByVal strSchemeCode As String, ByVal strvacclotno As String) As Boolean
        ...
        dtvaccinelotpendingrequest = udtVaccineLotBLL.GetVaccineLotPendingRequestList("", "", strvacclotno, strServiceType, strSchemeCode)
        ...
        If Not dtvaccinelotpendingrequest Is Nothing Then  'after + '.rowscount=0
            If dtvaccinelotpendingrequest.Rows.Count > 0 Then
                blnRequestPendingListRecordExist = True
            End If
        End If
        Return blnRequestPendingListRecordExist  'returen boolean xxxx
    End Function

    'check booth whether exist lot.no
    Private Function ckNonExistRecord(ByVal strServiceType As String, ByVal strSchemeCode As String, ByVal strVaccLotNo As String) As Boolean
        'Dim strNonExistList As String = Nothing
        ...
        dtVaccineLotExistLot = udtVaccineLotBLL.GetVaccineLotExistByServiceType(strServiceType, strSchemeCode, strVaccLotNo)
        If Not dtVaccineLotExistLot Is Nothing Then  'after + '.rowscount=0
            If dtVaccineLotExistLot.Rows.Count > 0 Then
                blnExist = True
            End If
        ...
        '            Exit For
        '        End If
        '    Next                                                   'check if booth has lot or not
    ...
    Private Function ckDuplicatedEffectiveDate(ByVal strServiceType As String, ByVal strSchemeCode As String, ByVal strVaccLotNo As String, ByVal strEffectiveFrom As String, ByVal strEffectiveTo As String) As Boolean
        Dim isDuplicatedRecord As Boolean = False
        'Dim boothDisplayItemList As StaticDataModelCollection = (New StaticDataBLL).GetStaticDataListByColumnName(BoothClassCode)
        ...
        Dim dtConfirmLotDetail As DataTable = udtVaccineLotBLL.GetVaccineLotExistByServiceType(strServiceType, strSchemeCode, strVaccLotNo) '????????
        Session(SESS_Confirm_HistoryView) = dtConfirmLotDetail

        If dtConfirmLotDetail.Rows.Count > 0 Then
            ...
            'Next
            If dvDuplicatedEffectiveDate.Count > 0 Then
...
#End Region
    ...
    Protected Sub gvSummary_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvSummary.SelectedIndexChanged
