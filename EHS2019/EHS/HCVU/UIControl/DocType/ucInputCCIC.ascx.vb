...
Public Class ucInputCCIC
 ...
 Private _strTDNumber As String
    ...
    Protected Overrides Sub RenderLanguage(ByVal strErrorImageURL As String, ByVal strErrorImageALT As String)
        ...
        ' ---------------------------------------------------------------------------------------------------------
        Me.lblDocumentTypeOriginal.Text = udtDocTypeModelList.Filter(DocTypeCode.CCIC).DocName(Session("Language").ToString())
        Me.lblTravelDocNoOriginalText.Text = udtDocTypeModelList.Filter(DocTypeCode.CCIC).DocIdentityDesc(Session("Language").ToString())

        ' -------------------------- Creation ------------------------------
        Me.lblNewTravelDocNoText.Text = udtDocTypeModelList.Filter(DocTypeCode.CCIC).DocIdentityDesc(Session("Language").ToString())
    ...
    Protected Overrides Sub Setup(ByVal modeType As ucInputDocTypeBase.BuildMode)
        ...
        ElseIf Me.Mode = BuildMode.Modification_OneSide Then
            ...
            Me._strTDNumber = MyBase.Formatter.FormatDocIdentityNoForDisplay(DocTypeCode.CCIC, MyBase.EHSPersonalInfoAmend.IdentityNum, False)
            Me._strENameFirstName = MyBase.EHSPersonalInfoAmend.ENameFirstName
            Me._strENameSurName = MyBase.EHSPersonalInfoAmend.ENameSurName
...
#Region "Set Up Text Box Value"
    ...
    Public Sub SetTDNumber()
        ...
        ElseIf Mode = BuildMode.Modification_OneSide Then
            lblNewTravelDocNo.Text = MyBase.Formatter.FormatDocIdentityNoForDisplay(DocTypeCode.CCIC, MyBase.EHSPersonalInfoOriginal.IdentityNum, False)
        Else
            'Amend
            Me.txtTDNo.Text = Me._strTDNumber
            Me.lblTDNo.Text = Me._strTDNumber
            'original
            lblTravelDocNoOriginal.Text = MyBase.Formatter.FormatDocIdentityNoForDisplay(DocTypeCode.CCIC, MyBase.EHSPersonalInfoOriginal.IdentityNum, False)
        End If
