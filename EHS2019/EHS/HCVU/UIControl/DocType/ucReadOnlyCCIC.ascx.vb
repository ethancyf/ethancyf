...
#End Region
    ...
    Public Sub Build(ByVal udtEHSPersonalInformation As EHSPersonalInformationModel, ByVal blnMaskTravelDocNo As Boolean, ByVal blnVertical As Boolean, ByVal intWidth As Integer, ByVal intWidth2 As Integer, Optional ByVal blnAlternateRow As Boolean = False)
        If blnVertical Then
            MultiViewCCIC.ActiveViewIndex = ViewIndex.Vertical

            ' Document Type 
            lblVDocumentType.Text = udtDocTypeBLL.getAllDocType().Filter(DocTypeCode.CCIC).DocName

            ' Travel Document No.
            lblVTravelDocNo.Text = udtFormatter.FormatDocIdentityNoForDisplay(DocTypeCode.CCIC, udtEHSPersonalInformation.IdentityNum, blnMaskTravelDocNo)
        ...
        Else
            MultiViewCCIC.ActiveViewIndex = ViewIndex.Horizontal

            ' Document Type 
            lblHDocumentType.Text = udtDocTypeBLL.getAllDocType().Filter(DocTypeCode.CCIC).DocName
