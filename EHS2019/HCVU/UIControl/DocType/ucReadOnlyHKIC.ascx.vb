...
#End Region
    ...
    Public Sub Build(ByVal udtEHSPersonalInformation As EHSPersonalInformationModel, ByVal blnMaskHKICNo As Boolean, ByVal blnVertical As Boolean, ByVal intWidth As Integer, ByVal intWidth2 As Integer, ByVal blnShowDateOfDeath As Boolean, ByVal blnShowDateOfDeathBtn As Boolean, ByVal blnShowCreationMethod As Boolean, Optional ByVal blnAlternateRow As Boolean = False)
        ...
        If blnVertical Then
            ...
            ' Document Type
            lblVDocumentType.Text = udtDocTypeBLL.getAllDocType().Filter(DocTypeCode.HKIC).DocName
            ...
            ' HKIC Date of Issue
            lblVHKIDIssueDate.Text = udtFormatter.formatDOI(DocTypeCode.HKIC, udtEHSPersonalInformation.DateofIssue)

            ' HKIC No.
            lblVHKIC.Text = udtFormatter.FormatDocIdentityNoForDisplay(DocTypeCode.HKIC, udtEHSPersonalInformation.IdentityNum, blnMaskHKICNo)
            ...
            If blnAlternateRow Then
                Alternate()
                tblVHKIC.Rows(0).Cells(1).Width = intWidth2
            End If


        Else
            MultiViewHKIC.ActiveViewIndex = ViewIndex.Horizontal

            ' Document Type
            lblHDocumentType.Text = udtDocTypeBLL.getAllDocType().Filter(DocTypeCode.HKIC).DocName
            ...
            ' HKIC No.
            lblHHKIC.Text = udtFormatter.FormatDocIdentityNoForDisplay(DocTypeCode.HKIC, udtEHSPersonalInformation.IdentityNum, blnMaskHKICNo)

            ' HKIC Date of Issue
            lblHHKIDIssueDate.Text = udtFormatter.formatDOI(DocTypeCode.HKIC, udtEHSPersonalInformation.DateofIssue)
            ...
            tblHHKIC.Rows(0).Cells(0).Width = intWidth
            tblHHKIC.Rows(1).Cells(1).Width = intWidth2
            tblHHKIC.Rows(1).Cells(2).Width = intWidth
    ...
    Protected Sub Alternate()
        Dim intRowCount = 0
        For Each row As HtmlTableRow In tblVHKIC.Rows
