...
SELECT	P.Display_Seq, P.SP_ID, P.Practice_Name, P.MO_Display_Seq,
		...
		P.Professional_Seq, 
		--P.Delist_Status as Practice_Delist_Status, --P.Remark as Practice_Remark, 
		...
		--	when 'A' then isNull(M.Upd_Type,P.Record_Status)
		--	--when 'S' then 'W'--P.Record_Status
		--	when 'S' then case isNull(M.Upd_Type, '')
		--						when 'RP' then isNull(M.Upd_Type,'')
		--						when 'DP' then 'TP'
		--						else 'W'
		--					  end
		--	when 'D' then isNull(P.Record_Status, '')
		--end [Practice_Record_Status],
		--
		--case P.Record_Status
		--	when 'A' then case isNull(M.Upd_Type,'')
		--						when '' then isNull(P.Remark,'')
	...
	--ON		P.SP_ID = M.SP_ID and M.Record_Status = 'A' and P.Display_Seq = M.SP_Practice_Display_Seq
