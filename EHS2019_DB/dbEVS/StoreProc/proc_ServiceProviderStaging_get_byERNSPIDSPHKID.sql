IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[proc_ServiceProviderStaging_get_byERNSPIDSPHKID]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[proc_ServiceProviderStaging_get_byERNSPIDSPHKID]
GO
...
-- Description:	
-- =============================================
CREATE PROCEDURE [dbo].[proc_ServiceProviderStaging_get_byERNSPIDSPHKID]
...
SELECT e.Enrolment_Ref_No, e.Enrolment_Dtm, e.SP_ID, convert(varchar, DecryptByKey(Encrypt_Field1)) as SP_HKID, convert(varchar(40), DecryptByKey(Encrypt_Field2)) as SP_Eng_Name, convert(nvarchar, DecryptByKey(Encrypt_Field3)) as SP_Chi_Name,
			...
			e.Fax, e.Email,	e.Already_Joined_EHR, e.Join_PCD, e.Application_Printed
...
GRANT EXECUTE ON [dbo].[proc_ServiceProviderStaging_get_byERNSPIDSPHKID] TO HCVU
GO
