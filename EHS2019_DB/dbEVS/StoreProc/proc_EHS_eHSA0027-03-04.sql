IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[proc_EHS_eHSA0027-03-04]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[proc_EHS_eHSA0027-03-04]
GO
...
CREATE PROCEDURE [dbo].[proc_EHS_eHSA0027-03-04]
	@Year	int
AS BEGIN
...
-- ---------------------------------------------
	...
	(0, 'eHSA0027-03: Monthly statistics of transactions claimed in ' +@strYear+', broken down by professions')
...
-- ---------------------------------------------
	...
	WHILE @i <= 12 BEGIN
		...
		SELECT @RNU = Transaction_Count FROM @Transaction WHERE Transaction_Month = @i AND Service_Type = 'RNU'
 		--SELECT @ROP = Transaction_Count FROM @Transaction WHERE Transaction_Month = @i AND Service_Type = 'ROP'
...
-- ---------------------------------------------
	...
	(0, 'eHSA0027-04: Monthly statistics of vouchers claimed in '+@strYear+', broken down by professions')
...
-- ---------------------------------------------
	...
	WHILE @i <= 12 BEGIN
		...
		--SELECT @ROP = Voucher_Count FROM @Transaction WHERE Transaction_Month = @i AND Service_Type = 'ROP' 
...
GRANT EXECUTE ON [dbo].[proc_EHS_eHSA0027-03-04] TO HCVU
GO
